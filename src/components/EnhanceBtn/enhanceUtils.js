...
export const getCurrentPlatform = () => {
  ...
  const found = Object.entries(PLATFORM_CONFIGS).find(([key, platform]) => platform.domain.split(".").every(part => host.includes(part)));
  return found ? found[0] : null;
};

/** Finds the container in which to position the button, if any. */
export const findPlatformButtonDiv = node => {
  const platform = getCurrentPlatform();
  if (!platform) return null;
  const domain = PLATFORM_CONFIGS[platform];
  //   let node = el;
  ...
  const fallbackArea = document.querySelector(domain.textareaSelector);
  if (fallbackArea) {
...
export const removeEnhanceButton = () => {
...
export const positionButton = () => {
  ...
  const platformButton = targetContainer.querySelector(domain.buttonSelector);
...
export function createEnhanceButton(container, textInput, platform, onEnhance) {
...
export async function handleEnhancePrompt(textInput) {
...
export async function extractPromptContent(textInput) {
  let content = "";
  const platform = getCurrentPlatform();

  console.log("extractPromptContent called:", {
    platform,
    textInput: textInput?.tagName,
    isContentEditable: textInput?.isContentEditable,
  });
  try {
    // Handle different types of input fields
    if (textInput.isContentEditable) {
